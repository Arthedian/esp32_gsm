...
modem_dte_t *esp_modem_dte_init(const esp_modem_dte_config_t *config, int tx_pin, int rx_pin, int rts_pin, int cts_pin)
{
    esp_err_t res;
    ...
    if (config->flow_control == MODEM_FLOW_CONTROL_HW) {
        res = uart_set_pin(esp_dte->uart_port, rx_pin, tx_pin,
                           rts_pin, cts_pin);
    } else {
        res = uart_set_pin(esp_dte->uart_port, rx_pin, tx_pin,
